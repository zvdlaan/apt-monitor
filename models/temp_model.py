...
def get_temps_all_stats():
	return db.query('SELECT COUNT(*) AS NumRowsInQuery, MIN(InsideTemp) AS InsideMin, MAX(InsideTemp) AS InsideMax, AVG(InsideTemp) As InsideMean, STD(InsideTemp) AS InsideStddev, MIN(OutsideTemp) AS OutsideMin, MAX(OutsideTemp) AS OutsideMax, AVG(OutsideTemp) As OutsideMean, STD(OutsideTemp) AS OutsideStddev FROM Temperature');
...
def get_temps_day_stats():
	return db.query('SELECT COUNT(*) AS NumRowsInQuery, MIN(InsideTemp) AS InsideMin, MAX(InsideTemp) AS InsideMax, AVG(InsideTemp) As InsideMean, STD(InsideTemp) AS InsideStddev, MIN(OutsideTemp) AS OutsideMin, MAX(OutsideTemp) AS OutsideMax, AVG(OutsideTemp) As OutsideMean, STD(OutsideTemp) AS OutsideStddev FROM Temperature WHERE MeasuredTime >= DATE(NOW())');    
...
def get_temps_week_stats():
	return db.query('SELECT COUNT(*) AS NumRowsInQuery, MIN(InsideTemp) AS InsideMin, MAX(InsideTemp) AS InsideMax, AVG(InsideTemp) As InsideMean, STD(InsideTemp) AS InsideStddev, MIN(OutsideTemp) AS OutsideMin, MAX(OutsideTemp) AS OutsideMax, AVG(OutsideTemp) As OutsideMean, STD(OutsideTemp) AS OutsideStddev FROM Temperature WHERE MeasuredTime >= DATE(NOW()) - INTERVAL 1 WEEK');    
